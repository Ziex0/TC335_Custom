...
static bool Arena1v1CheckTalents(Player* player)
{
    ...
    if (count >= 36)
    {
        ChatHandler(player->GetSession()).SendSysMessage("Du kannst nicht mitmachen, weil du zu viele Punkte in ein verbotenes Talent investiert hast. Bitte bearbeiten Sie Ihre Talente.");
        return false;
    }
    else
        return true;
