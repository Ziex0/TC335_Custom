...
public:
    ...
    struct npc_1v1arenaAI : public ScriptedAI
    {
        ...
        bool CreateArenateam(Player* player, Creature* me)
        {
            ...
            ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[PvP Arena System]:|r Arena Team wurde erfolgreich erstellt!");
        ...
        bool OnGossipHello(Player* player)
        {
            ...
            {
                ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[PvP Arena System]:|r PvP Arena ist nicht aktiv!");
                return true;
            }

            if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5))
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Verlasse PvP Arena Warteschlange", GOSSIP_SENDER_MAIN, 3, "Are you sure?", 0, false);
            else
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Ein PvP Arena Team anmelden (Nicht gewertet)", GOSSIP_SENDER_MAIN, 20);

            if (player->GetArenaTeamId(ArenaTeam::GetSlotByType(ARENA_TEAM_5v5)) == 0)
                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Erstelle neues PvP Arena Team", GOSSIP_SENDER_MAIN, 1, "PvP Arena Team erstellen?", config_Costs, false);
            else
            {
                if (player->InBattlegroundQueueForBattlegroundQueueType(BATTLEGROUND_QUEUE_5v5) == false)
                {
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Ein PvP Arena Team anmelden (Gewertet)", GOSSIP_SENDER_MAIN, 2);
                    AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Team auflösen", GOSSIP_SENDER_MAIN, 5, "Bist du dir da sicher?", 0, false);
                }

                AddGossipItemFor(player, GOSSIP_ICON_CHAT, "Statistiken ansehen", GOSSIP_SENDER_MAIN, 4);
            }
        ...
        bool OnGossipSelect(Player* player, uint32 /*menuId*/, uint32 gossipListId)
        {
            ...
            case 1: // Create new Arenateam
            {
                ...
                {
                    ChatHandler(player->GetSession()).PSendSysMessage("|cffFF4500[PvP Arena System]:|r Du benötigst Stufe %u um dein Arena Team anmelden zu können.", config_minLevel);
            ...
            case 2: // Join Queue Arena (rated)
            {
                if (Arena1v1CheckTalents(player) && JoinQueueArena(player, me, true) == false)
                    ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[PvP Arena System]:|r Beim Beitritt zur Warteschlange ist etwas schief gelaufen.");
            ...
            case 4: // get statistics
            {
                ...
                {
                    std::stringstream s;
                    s << "|cffFF4500[PvP Arena System]:|r Your statistics ";
            ...
            case 5: // Disband arenateam
            {
                ...
                ChatHandler(player->GetSession()).SendSysMessage("|cffFF4500[PvP Arena System]:|r Arena Team gelöscht!");
                CloseGossipMenuFor(player);
                return true;
            }
            break;
